...
class BenchmarkRunner:
    ...
    def benchmark_unwrap(
        self, name: str, vertices: np.ndarray, triangles: np.ndarray, k_star: float = 0.721
    ) -> dict:
        """
        ...
        print(
            f"  Stitch edges: {len(atlas.stitch_edges)} ({100 * len(atlas.stitch_edges) / len(atlas.edges):.1f}%)"
        )
        print(
            f"  Angle distortion: {angle_dist['mean']:.2f}° (mean), {angle_dist['max']:.2f}° (max)"
        )
