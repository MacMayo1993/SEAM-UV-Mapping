...
class NonOrientableUnwrapOperator(bpy.types.Operator):
    ...
    verbose: bpy.props.BoolProperty(
        name="Verbose", description="Print progress messages to console", default=False
    )
    ...
    def execute(self, context):
        ...
        try:
            ...
            if self.verbose:
                self.report(
                    {"INFO"}, f"Processing {len(vertices)} vertices, {len(triangles)} triangles"
                )

            # Run algorithm
            atlas = TopologicalUVAtlas(
                vertices, triangles, k_star=self.k_star, verbose=self.verbose
            )
...
class CreateParityShaderOperator(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if "parity" not in mesh.attributes:
            self.report(
                {"WARNING"}, "Parity attribute not found. Run Non-Orientable UV Unwrap first."
            )
            return {"CANCELLED"}
